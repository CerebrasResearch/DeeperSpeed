...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        #Cerebras changes made here for logging zloss and celoss
        self.ce_loss = torch.tensor(0.0).to(self.device)
        #stores the loss for the entire batch
        self.total_ce_loss = None
        self.agg_ce_loss = torch.tensor(0.0, requires_grad=False).to(self.device)
        self.dp_group_ce_loss = torch.tensor(0.0, requires_grad=False).to(self.device)

        self.z_loss = torch.tensor(0.0).to(self.device)
        #stores the loss for the entire batch
        self.total_z_loss = None
        self.agg_z_loss = torch.tensor(0.0, requires_grad=False).to(self.device)
        self.dp_group_z_loss = torch.tensor(0.0, requires_grad=False).to(self.device)

        self.aux_values = None
    ...
    # Cerebras changes made here
    def train_batch(self, data_iter=None, aux_values=None):
        ...
        Args:
            ...
            aux_values: a dictionary to query auxillary training values
        ...
        if self.aux_values is not None:
            self.agg_train_ce_loss = self._aggregate_total_ce_loss()
            self.agg_train_z_loss = self._aggregate_total_z_loss()
        ...
        if aux_values is not None:
            aux_values["ce_loss"] = self.agg_train_ce_loss
            aux_values["z_loss"] = self.agg_train_z_loss
    ...
    # Cerebras Changes made here
    def _aggregate_total_ce_loss(self):
        # Scale ce_loss, average among DP ranks, and bcast ce_loss to the rest of my DP group
        if self.is_last_stage():
            ce_loss = self._scale_loss_by_gas(self.total_ce_loss)
            self.dp_group_ce_loss = ce_loss.clone().detach()

            ## Average loss across all data-parallel groups
            agg_ce_loss = self.dp_group_ce_loss.clone().detach()
            #print(f'RANK={self.global_rank} bcast SENDER src={self.global_rank} group={self.grid.pp_group}', flush=True)
            if self.is_data_parallel:
                dist.all_reduce(agg_ce_loss, group=self.mpu.get_data_parallel_group())
                agg_ce_loss /= self.dp_world_size

            assert self.global_rank in self.grid.pp_group
            ce_losses = torch.Tensor([self.dp_group_ce_loss, agg_ce_loss]).to(self.device)
            dist.broadcast(tensor=ce_losses, src=self.global_rank, group=self.mpu.get_pipe_parallel_group())

        else:
            # Get loss from last stage
            src_rank = self.grid.stage_to_global(self.num_stages - 1)
            assert src_rank in self.grid.pp_group
            ce_losses = torch.Tensor([0., 0.]).to(self.device)
            dist.broadcast(tensor=ce_losses, src=src_rank, group=self.grid.get_pipe_parallel_group())
            self.dp_group_ce_loss = ce_losses[0].clone().detach()
            agg_ce_loss = ce_losses[1].clone().detach()

        return agg_ce_loss
    
    def _aggregate_total_z_loss(self):
        # Scale z_loss, average among DP ranks, and bcast z_loss to the rest of my DP group
        if self.is_last_stage():
            z_loss = self._scale_loss_by_gas(self.total_z_loss)
            self.dp_group_z_loss = z_loss.clone().detach()

            ## Average loss across all data-parallel groups
            agg_z_loss = self.dp_group_z_loss.clone().detach()
            #print(f'RANK={self.global_rank} bcast SENDER src={self.global_rank} group={self.grid.pp_group}', flush=True)
            if self.is_data_parallel:
                dist.all_reduce(agg_z_loss, group=self.mpu.get_data_parallel_group())
                agg_z_loss /= self.dp_world_size

            assert self.global_rank in self.grid.pp_group
            z_losses = torch.Tensor([self.dp_group_z_loss, agg_z_loss]).to(self.device)
            dist.broadcast(tensor=z_losses, src=self.global_rank, group=self.mpu.get_pipe_parallel_group())

        else:
            # Get loss from last stage
            src_rank = self.grid.stage_to_global(self.num_stages - 1)
            assert src_rank in self.grid.pp_group
            z_losses = torch.Tensor([0., 0.]).to(self.device)
            dist.broadcast(tensor=z_losses, src=src_rank, group=self.grid.get_pipe_parallel_group())
            self.dp_group_z_loss = z_losses[0].clone().detach()
            agg_z_loss = z_losses[1].clone().detach()
        ...
        if self.is_last_stage():
            if self._compute_loss and self.module.loss_fn is not None:
                ...
                if self.aux_values is None:
                    ...
                    self.loss = self.module.loss_fn(outputs, labels, aux_values=self.aux_values)
                    self.ce_loss = self.aux_values["ce_loss"]
                    self.z_loss = self.aux_values["z_loss"]
            ...
            if isinstance(self.loss, torch.Tensor):
                ...
                if self.aux_values is not None:
                    ...
                    self.fwd_outputs.append(self.ce_loss.detach())
                    self.fwd_outputs.append(self.z_loss.detach())

                    if self.total_ce_loss is None:
                        self.total_ce_loss = torch.zeros_like(self.ce_loss)
                    self.total_ce_loss += self.loss.detach()

                    if self.total_z_loss is None:
                        self.total_z_loss = torch.zeros_like(self.z_loss)
                    self.total_z_loss += self.z_loss.detach()
            else:
                ...
                # Cerebras changes made here
                if self.aux_values is not None:
                    self.fwd_outputs.append([l.detach() for l in self.ce_loss])

                    if self.total_ce_loss is None:
                        self.total_ce_loss = [torch.zeros_like(l) for l in self.ce_loss]
                    for idx, l in enumerate(self.ce_loss):
                        self.total_ce_loss[idx] += l.detach()

                    self.fwd_outputs.append([l.detach() for l in self.z_loss])

                    if self.total_z_loss is None:
                        self.total_z_loss = [torch.zeros_like(l) for l in self.z_loss]
                    for idx, l in enumerate(self.z_loss):
                        self.total_z_loss[idx] += l.detach()
